...
public class RepositorioServico : IRepositorioServico
{
    ...
    public async Task<IReadOnlyList<ServicoCatalogo>> ObterTodosAsync(CancellationToken cancelamento = default)
    {
    ...
    public async Task<ServicoCatalogo?> ObterPorIdAsync(string id, CancellationToken cancelamento = default)
    {
        return await _contexto.Servicos.FirstOrDefaultAsync(servico => servico.Id == id, cancelamento);
    }

    public async Task<ServicoCatalogo?> ObterPorNomeAsync(string nome, CancellationToken cancelamento = default)
    {
        var termo = nome.Trim().ToLower();

        return await _contexto.Servicos
            .AsNoTracking()
            .FirstOrDefaultAsync(servico => servico.Nome.ToLower() == termo, cancelamento);
    }

    public async Task<ServicoCatalogo> CriarAsync(ServicoCatalogo servico, CancellationToken cancelamento = default)
    {
    ...
    public async Task<ServicoCatalogo?> AtualizarAsync(ServicoCatalogo servico, CancellationToken cancelamento = default)
    {
